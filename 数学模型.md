...
定义原始权重：
$
W_{\text{raw}}(k) = \begin{cases}
2 \cdot \text{Current}(k) - \text{Expected}(k) & \text{if } \text{Current}(k) > \text{Expected}(k) \\
0 & \text{otherwise}
\end{cases}
$
...
归一化权重：
$
W(k) = \frac{W_{\text{raw}}(k)}{\sum_{k' \in \mathcal{A}} W_{\text{raw}}(k')}
$
...
**示例（按张数占比）**：
- 期望分布：大额 0.5（50%张数），中额 0.4（40%张数），小额 0.1（10%张数）
- 实际分布：大额 0.3（30%张数），中额 0.5（50%张数），小额 0.2（20%张数）
- 计算原始权重：
  - 大额：$0.3 \not> 0.5$，权重 = 0
  - 中额：$0.5 > 0.4$，权重 = $2 \times 0.5 - 0.4 = 0.6$
  - 小额：$0.2 > 0.1$，权重 = $2 \times 0.2 - 0.1 = 0.3$
- 归一化：总权重 = $0 + 0.6 + 0.3 = 0.9$
- 最终得分：大额 $0/0.9 = 0$，中额 $0.6/0.9 \approx 0.67$，小额 $0.3/0.9 \approx 0.33$
- 优先消耗顺序：中额 > 小额 > 大额

该策略优先消耗超出期望占比的标签，权重同时考虑当前总量和超出量，使库存趋向期望分布。
...
定义差额：
$$
\mathrm{bias\_amount} = M - \sum_{i=1}^{N} x_i \cdot u_i
$$
...
**拆票使能约束：**
$$
\alpha_i < 1 \Rightarrow \mathrm{split\_allowed} = \text{True}
$$
...
$$
\text{if } 0 < \mathrm{bias\_amount} < \mathrm{tail\_diff} \Rightarrow \text{不拆票，走电汇补齐}
$$
...
$$
\text{if } \mathrm{bias\_amount} > 0 \text{ and } \mathrm{bias\_amount} > \mathrm{tail\_diff}
$$
...
拆票得分根据拆票策略 $\mathrm{split\_strategy}$ 计算，可以基于以下维度：
...
\frac{1}{v_i - |\mathrm{bias\_amount}|} & \text{if } v_i \geq |\mathrm{bias\_amount}| \\
0 & \text{otherwise}
\end{cases}
$$
...
**场景1（补票）：**
$$
\alpha_{j^*} = \min\left(1, \frac{\mathrm{bias\_amount}}{v_{j^*}}\right)
$$
...
| 差额 | $\mathrm{bias\_amount} = M - \sum_{i: x_i=1} u_i$ | 与付款单金额的差额 |
