...
该策略根据票据类型的库存分布和用户期望库存分布，计算得到票据类型偏好。

**注意**：库存分布按票据张数占比计算，而非金额占比。

计算公式：
- 对于实际占比 > 期望占比的标签：原始权重 = 2 × 实际占比 - 期望占比
- 对于实际占比 ≤ 期望占比的标签：原始权重 = 0
- 最终得分 = 原始权重 / 所有标签原始权重之和

示例：
```
用户期望票据分布（按张数）：大额 0.5, 中额 0.4, 小额 0.1
实际初始库存分布（按张数）：大额 0.3, 中额 0.5, 小额 0.2
...
该策略优先消耗超出期望占比的票据，权重同时考虑当前总量和超出量，使库存趋向期望分布。
